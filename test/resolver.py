...
from dnslib import RR, RDMAP, RD
...
class FixedResolver(BaseResolver):
    ...
    def __init__(self,zone):
        # Parse RRs
        IPN = RD(data=(1).to_bytes(8))
        print(IPN)
        self.rrs = [RR(rname='.', rtype=264, ttl=3600, rdata=IPN)]
        # self.rrs = RR.fromZone(zone)
        print(self.rrs)
        print(type(self.rrs[0].rtype))
        print(type(self.rrs[0].rdata))
...
if __name__ == '__main__':
    ...
    IPN = RD(data=(1).to_bytes(8))
    print(IPN)
    print(RR(rname='.', rtype=264, ttl=3600, rdata=IPN))
