...
static int bpmailsend(void) {
    ...
    SdrObject adu_payload = sdr_insert(sdr, content, (size_t)content_size);
    if (sdr_end_xn(sdr) != 0) {
    ...
    //SdrObject adu_zco = ionCreateZco(
    //    ZcoSdrSource,
    //    adu_payload,
    //    0,
    //    content_size,
    ...
    //);
    //if (adu_zco == 0 || adu_zco == (SdrObject)ERROR) {
    ...
    if (dtpc_send(
            ...
            dest_eid,
            /*
             * If profile_id is 0, retransmit a single DTPC ADU at most once
             * and disable aggregation and elision.
             */
            1,
            0,
            0,
        ...
        <= 0)
    {
        (void)fprintf(stderr, "could not send DTPC application data unit\n");
        free(content);
        return EXIT_FAILURE;
    }
